...
export default function MyOrder() {
    ...
    const fetchMyOrder = async () => {
        ...
        await fetch(`${BASE_URL}/api/auth/myOrderData`, {
            // credentials: 'include',
        ...
        <div>
            ...
            <div className='container'>
                <div className='row'>
                    ...
                    {orderData !== {} ? Array(orderData).map((data, dataIdx) => {
  return (
    data.orderData ?
      data.orderData.order_data.slice(0).reverse().map((item, orderIdx) => {
        return (
          <div key={`order-${orderIdx}`} className="mb-5">
            {item.map((arrayData, itemIdx) => (
